...
Do not attempt this. The later version of RHACM (2.1) will allow you to manage the Hub clusters.
...
Bare metal is included as a tech preview feature in 2.0. For 2.1 we will have made this feature GA and add vSphere as a supported deploying platform.
...
. Select the node pools and expand it. Change the default provisoning zone from *us-east-1* to *us-west-1*
. The rest of the information will auto fill, verify that it does. Click **Create**.

[NOTE]
====
The creation of a cluster takes approximately 30-45 minutes, it can take longer depending on demand. Please continue with the rest of the lab.
====
...
. Open a terminal window and use SSH to access the `bastion` VM of your _managed_ OpenShift cluster. You can also use the Terminal within the Bookback instructions.
